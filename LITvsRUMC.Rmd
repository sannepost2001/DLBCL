...
genenames_per_case_plot <- function(chars, mysheets, amountcases) {
  ...
  genesOrderdPercent$percentage <- (genesOrderdPercent$n/amountcases)*100
...
genesOrderd_all <- genenames_per_case_plot("", mysheets, 11)
plot(genesOrderd_all, 20)

# Get the genes of relapse and plot these
genesOrderd_rel <- genenames_per_case_plot("rel", mysheets, 11)
head(genesOrderd_rel, n = 20, n) %>%
  ggplot(., aes(x = reorder(`Gene name` ,-percentage), y = percentage)) +
  theme(axis.text.x = element_text(
    angle = 90,
    vjust = 0.5,
...
genesOrderd_diag <- genenames_per_case_plot("diag", mysheets, 11)
head(genesOrderd_diag, n = 20, n) %>%
  ggplot(., aes(x = reorder(`Gene name` ,-percentage), y = percentage)) +
  theme(axis.text.x = element_text(
    angle = 90,
    vjust = 0.5,
